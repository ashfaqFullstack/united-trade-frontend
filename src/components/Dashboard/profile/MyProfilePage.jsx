...
import { useMyPendingProfileUpdate } from '@/hooks/useProfileUpdate';
...
export default function MyProfilePage() {
    ...
    const { data: pendingRequest, isLoading: isPendingLoading } = useMyPendingProfileUpdate();

    if (!user) return null;

    const profile = isBusiness ? businessProfile : customerProfile;
    const isLoading = (isBusiness ? isBusinessLoading : isCustomerLoading) || isPendingLoading;

    if (isLoading) return <Loading />;

    // A profile update is already awaiting admin review — don't let the
    // user edit again (or see the normal overview) until it's resolved.
    if (pendingRequest) {
            ...
            {isEditing ? (
                <>
                    <div className="flex items-center justify-between">
                        ...
                        <BackButton handleBack={() => setIsEditing(false)} title="Back" />
                    </div>
